...
public class CustomerPR {
	...
	@DataProvider
	public void getCustomer(Page<Customer> page,Criteria criteria) throws Exception{
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(Customer.class);
		if(criteria != null){
			HibernateUtils.createFilter(detachedCriteria, criteria);
		}
		customerDao.find(page, detachedCriteria.getExecutableCriteria(customerDao.getSession()));
	}
	@DataProvider
	...
	public String validatorCompanyName(String name){
		if(StringUtils.hasText(name)){
			if(customerDao.findUnique("from Customer where companyName=?", new Object[]{name})!=null){
				return "客户名称重复";
			}
		}
		return null;
	...
	public void saveAll(List<Customer> customers){
		customerDao.persistEntities(customers);
